...
function isRateTool(tool) {
  ...
  return (names.has("from_currency") && names.has("to_currency")) ||
    names.has("symbol") ||
...
function buildParameters(tool, from, to, amount, date) {
  ...
  if (names.has("function")) params.function = "CURRENCY_EXCHANGE_RATE";
...
function parseArgs(argv) {
  ...
  const parsed = {
    command: null, from: null, to: null, amount: null,
    date: null, format: "markdown", timeoutMs: DEFAULT_TIMEOUT_MS,
  };
...
async function runRate(args) {
  const { from, to, date, timeoutMs } = args;
  ...
  if (!candidates.length) return { from, to, rate: null, converted: null, amount: null, pair: `${from}/${to}`, error: "No rate tools found." };
  candidates.sort((a, b) => scoreTool(b, false) - scoreTool(a, false));
  for (const tool of candidates) {
    const params = buildParameters(tool, from, to, null, date);
    ...
    try {
      ...
      const extracted = extractRateAndConverted(result?.result ?? result, from, to, null);
      return { from, to, rate: extracted.rate, converted: extracted.converted, amount: null, pair: extracted.pair, tool_id: tool.tool_id };
    } catch (_) { continue; }
  }
  return { from, to, rate: null, converted: null, amount: null, pair: `${from}/${to}`, error: "All rate lookups failed." };
}

async function runConvert(args) {
  const { from, to, amount, date, timeoutMs } = args;
  ...
  for (const query of SEARCH_QUERIES) {
    const searchResult = await searchTools(query, 15, timeoutMs + 2000);
    for (const tool of (searchResult.results ?? [])) {
      if (!tool?.tool_id || (!isConversionTool(tool) && !isRateTool(tool))) continue;
      if (candidates.some((c) => c.tool_id === tool.tool_id)) continue;
      candidates.push({ ...tool, search_id: searchResult.search_id });
    }
  }
  if (!candidates.length) return { from, to, rate: null, converted: null, amount: amountNum, pair: `${from}/${to}`, error: "No conversion tools found." };
  ...
  for (const tool of candidates) {
    ...
    try {
      const result = await executeTool(tool.tool_id, tool.search_id, params, MAX_RESPONSE_SIZE, timeoutMs);
      if (!isExecutionSuccess(result)) continue;
      const extracted = extractRateAndConverted(result?.result ?? result, from, to, amountNum);
      return { from, to, rate: extracted.rate, converted: extracted.converted, amount: amountNum, pair: extracted.pair, tool_id: tool.tool_id };
    } catch (_) { continue; }
  }
  return { from, to, rate: null, converted: null, amount: amountNum, pair: `${from}/${to}`, error: "All conversions failed." };
}
...
async function main() {
  ...
  if (args.help || !args.command) { printHelp(); return; }
  let output;
  if (args.command === "rate") output = await runRate(args);
  else if (args.command === "convert") output = await runConvert(args);
  else throw new Error(`Unknown command: ${args.command}`);
  console.log(args.format === "json" ? JSON.stringify(output, null, 2) : formatMarkdown(output));
}
